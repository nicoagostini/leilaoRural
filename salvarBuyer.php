<?php
	...
	if($uc->controleAcao("validaCPF", $cpf) == false){
		header("location:cadastraBuyer.php?error=3");
		die;
	}

	if($nome == '' || $email == '' || $senha == '' || $senha2 == '' || $recebe['name'] == '' || $rg == '' || $cpf == '' || $apelido == '' || $endereco == '' || $cidade == '' || $estado == '' || $cc == '' || $data == '' || $telefone == '' || $dataNascimento == '' || $sexo == '' || $ec == '' ){
		header("location:cadastraBuyer.php?error=1");
		die;
	}

	elseif($senha != $senha2){
		header("location:cadastraBuyer.php?error=2");
		die;
	}
	...
	$bc->controleAcao("inserirBuyer",$buyer);
